...
plot2 <- ggplot(aggregate_word_frequency_by_section, aes(section, percentage)) +
  geom_line(color = "#ED1C24") +
  scale_x_continuous(limits = c(1,39), breaks = seq(0, 100), expand = c(0, 0)) +
  theme(axis.title.y = element_text(vjust = 2, family = "mono"),
        axis.title.x = element_text(family = "mono"))
...
Now I look at the frequency of specific words across chapters/sections:

```{r}
# calculating frequency of specified words across chapters/sections:
...
word_pairs_by_chapter <- specified_words_filtered |>
  select(chapter, word) |>
  group_by(word) |>
  pairwise_cor(word, chapter, sort = TRUE)

word_pairs_by_section <- specified_words_filtered |>
  select(section, word) |>
  group_by(word) |>
...
plot1 <- word_pairs_by_chapter |>
  ...
  geom_node_point(color = "#ED1C24", size = 3) +
